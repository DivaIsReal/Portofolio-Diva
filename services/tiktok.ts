...
// const profileFields = "avatar_large_url,display_name,...";
// const videoFields = "id,create_time,...";

// export async function getStoredToken() { ... } // DISABLED
// export async function saveTikTokTokens(tokenData: any) { ... } // DISABLED
// export async function refreshTikTokToken(refreshToken: string) { ... } // DISABLED
// export async function getValidAccessToken() { ... } // DISABLED

export async function getTikTokProfile() {
  return null; // DISABLED: TikTok API + Supabase
}
...
export async function getTikTokVideos(
  ...
  return { videos: [], has_more: false, cursor: 0 }; // DISABLED: TikTok API + Supabase
}

// ======================================================
// KODE ASLI - Uncomment semua di bawah ini untuk re-enable
// ======================================================
...
//   const refreshExpiresAt = new Date(Date.now() + (tokenData.refresh_expires_in || 15552000) * 1000).toISOString();
...
//   );
//   if (!response.data.access_token) throw new Error("Gagal mendapatkan token baru.");
...
//   if (isExpired) return await refreshTikTokToken(record.refresh_token);
...
// }
//
// export async function getTikTokVideos(cursor: number = 0, maxCount: number = 10) {
//   const token = await getValidAccessToken();
//   const response = await axios.post(
